...
_z3_op_to_str = {
    ...
    Z3_OP_BNOT : 'not', Z3_OP_BXOR : '^', Z3_OP_BNEG : '-', Z3_OP_BUDIV : 'UDiv', Z3_OP_BSDIV : '/', Z3_OP_BSMOD : '%',
...
def _is_zustre_assoc(k):
    return k == Z3_OP_AND or k == Z3_OP_OR or k == Z3_OP_IFF or _is_assoc(k)

def _is_zustre_left_assoc(k):
    return _is_zustre_assoc(k) or k == Z3_OP_SUB or k == Z3_OP_BSUB
...
_zustre_ellipses = '&hellip;'
# Overwrite some of the operators for ZUSTRE
_z3_pre_zustre_op_to_str = {Z3_OP_TRUE : 'true', Z3_OP_FALSE : 'false',
                           Z3_OP_EQ : '=', Z3_OP_IFF : '=', Z3_OP_NOT : 'not ',
                           Z3_OP_AND : 'and ', Z3_OP_OR : 'or ', Z3_OP_IMPLIES : '=>',
                           Z3_OP_LT : '<', Z3_OP_GT : '>', Z3_OP_LE : '<=', Z3_OP_GE : '>=',
                           Z3_OP_MUL : '*',
                           Z3_OP_SLEQ : '<=', Z3_OP_SLT : '<', Z3_OP_SGEQ : '>=', Z3_OP_SGT : '>'
                           }

# Extra operators that are infix/unary for ZUSTRE
_z3_zustre_infix = [ Z3_OP_AND, Z3_OP_OR, Z3_OP_IMPLIES,
                   Z3_OP_ULEQ, Z3_OP_ULT, Z3_OP_UGEQ, Z3_OP_UGT, Z3_OP_BUDIV, Z3_OP_BUREM, Z3_OP_BLSHR
                   ]

_z3_zustre_unary = [ Z3_OP_NOT ]

# Extra Precedence for ZUSTRE
_z3_pre_zustre_precedence = { Z3_OP_BUDIV : 2, Z3_OP_BUREM : 2,
                            Z3_OP_BLSHR : 4,
...
_z3_zustre_op_to_str = {}
for _k in _z3_op_to_str:
    _v = _z3_op_to_str[_k]
    _z3_zustre_op_to_str[_k] = _v
for _k in _z3_pre_zustre_op_to_str:
    _v = _z3_pre_zustre_op_to_str[_k]
    _z3_zustre_op_to_str[_k] = _v

_z3_zustre_precedence = {}
for _k in _z3_precedence:
    _v = _z3_precedence[_k]
    _z3_zustre_precedence[_k] = _v
for _k in _z3_pre_zustre_precedence:
    _v = _z3_pre_zustre_precedence[_k]
    _z3_zustre_precedence[_k] = _v

_zustre_infix_map = {}
_zustre_unary_map = {}
for _k in _z3_infix:
    _zustre_infix_map[_k] = True
for _k in _z3_zustre_infix:
    _zustre_infix_map[_k] = True
for _k in _z3_unary:
    _zustre_unary_map[_k] = True
for _k in _z3_zustre_unary:
    _zustre_unary_map[_k] = True

def _is_zustre_infix(k):
    global _zustre_infix_map
    return _zustre_infix_map.get(k, False)

def _is_zustre_unary(k):
    global _zustre_unary_map
    return _zustre_unary_map.get(k, False)

def _zustre_op_name(a):
    global _z3_zustre_op_to_str
    if isinstance(a, z3.FuncDeclRef):
        f = a
    else:
        f = a.decl()
    k = f.kind()
    n = _z3_zustre_op_to_str.get(k, None)
    if n == None:
...
def _get_zustre_precedence(k):
    global _z3_zustre_predence
    return _z3_zustre_precedence.get(k, 100000)
...
class ZUSTREFormatter(Formatter):
    def __init__(self):
        Formatter.__init__(self)
        global _zustre_ellipses
        self.ellipses = to_format(_zustre_ellipses)
    ...
    def is_assoc(self, k):
        return _is_zustre_assoc(k)

    def is_left_assoc(self, k):
        return _is_zustre_left_assoc(k)

    def is_infix(self, a):
        return _is_zustre_infix(a)

    def is_unary(self, a):
        return _is_zustre_unary(a)

    def get_precedence(self, a):
        return _get_zustre_precedence(a)
