...
% MODEL_INFOS Retrieve the informations from the model provided as input
	...
	handled_blocks.classic = {'Inport', 'Outport', 'Gain', 'Abs', 'Logic', 'Product', 'MinMax', 'Switch', 'Sum', 'Saturate', 'SubSystem', 'Relatio1nalOperator', 'UnitDelay', 'Mux', 'Demux', 'Constant', 'Trigonometry', 'Math', 'DiscreteIntegrator', 'Dynamic Saturation', 'DiscreteZeroPole', 'DiscreteStateSpace', 'Goto', 'From', 'Concatenate', 'MultiPortSwitch', 'Reshape', 'Terminator', 'Sqrt', 'Selector', 'Assignment', 'ActionPort', 'EnablePort', 'TriggerPort', 'Bias', 'Reshape', 'SwitchCase', 'Memory', 'DataTypeConversion', 'ForIterator', 'Polyval', 'LookupNDDirect', 'SignalSpecification', 'BusCreator', 'BusSelector', 'BusAssignment', 'Bitwise Operator', 'ToWorkspace', 'ModelReference', 'S-Function'};
    handled_blocks.masked = {'Bitwise Operator', 'Saturation Dynamic', 'DiscretizedZeroPole', 'Compare To Zero', 'Compare To Constant', 'Detect Change', 'Detect Increase', 'Detect Decrease', 'Detect Rise Positive', 'Detect Rise Nonnegative', 'Detect Fall Negative', 'Detect Fall Nonpositive', 'Observer'};
...
function [add_blocks stat_support_out] = get_sub_blocks(subsys_name, root, stat_support)
	...
	stat_support{idx_stat_support}.string = sprintf('INFO: {Model: %s; Block: %3.2f%%; Block_type: %3.2f%%}', stat_support{idx_stat_support}.block_name, stat_support{idx_stat_support}.percent_bl, stat_support{idx_stat_support}.percent_bt);
