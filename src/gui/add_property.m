...
uicontrol(upper_panel,'Style','text',...
    ...
    'FontUnits', 'Normalized', 'FontSize',space, 'FontWeight', 'bold', ...
    'Units', 'Normalized','Position',[0.05 0.65 0.2 space]);
...
subsystems_popup = uicontrol(upper_panel,'Style','popup',...
    ...
    'FontUnits', 'Normalized', 'FontSize',space, 'FontWeight', 'bold', ...
    'Units', 'Normalized','Position',[0.3 0.65 0.6 space],...
...
uicontrol(upper_panel,'Style','text',...
    'String','Inports names :',  ...
    'HorizontalAlignment','left',...
    'FontUnits', 'Normalized', 'FontSize',space, 'FontWeight', 'bold', ...
    'Units', 'Normalized','Position',[0.05 0.25 0.2 space]);

first_inports = Utils.concat_delim(get_inports(file_name), ', ');
inports_handle = uicontrol(upper_panel,'Style','text',...
    'String', first_inports,  ...
    'HorizontalAlignment','left',...
    'FontUnits', 'Normalized', 'FontSize',space, 'FontWeight', 'bold', ...
    'Units', 'Normalized','Position',[0.3 0.25 0.2 space]);


uicontrol(upper_panel,'Style','text',...
    'String','Assumptions: (Example: assume In1 >= 0.0;)','HorizontalAlignment','left',...
    'FontUnits', 'Normalized', 'FontSize',space, 'FontWeight', 'bold', ...
    'Units', 'Normalized','Position',[0.05 0.15 0.2 space]);

annotations_handle = uicontrol(upper_panel,'Style','edit',...
    'String','','HorizontalAlignment','left',...
    'Min',0, 'Max', 100, ...
    'FontUnits', 'Normalized', 'FontSize',space, 'FontWeight', 'bold', ...
    'Units', 'Normalized','Position',[0.3 0.15 0.6 space]);
...
uicontrol(upper_panel,'Style','pushbutton',...
    ...
    'FontUnits', 'Normalized', 'FontSize',space, 'FontWeight', 'bold', ...
    'Units', 'Normalized','Position',[0.05 0.01 0.1 0.1],...
    'Callback', @create_pp)

uicontrol(upper_panel,'Style','pushbutton',...
    'String','Close','HorizontalAlignment','left',...
    'FontUnits', 'Normalized', 'FontSize',space, 'FontWeight', 'bold', ...
    'Units', 'Normalized','Position',[0.15 0.01 0.1 0.1],...
    'Callback', @close)
%% call backs
    ...
    function setOutports(source,callbackdata)
        ...
        inports_names = get_inports(subsystem_name);
        inports_handle.String = Utils.concat_delim(inports_names, ', ');
    end
...
function inports_names = get_inports(subsystem_name)
try
    inports = find_system(subsystem_name, 'SearchDepth', '1', 'Type', 'Block', 'BlockType', 'Inport');
catch ME
    errordlg(ME.message);
    display_msg(ME.getReport(),Constants.DEBUG, 'add property','');
    inports_names{1} = 'No available outputs.';
    inports = {};
    return;
end
if numel(inports) > 0
    % Display the list of available outports for thie Subsystem
    inports_names = cell(numel(inports),1);
    for idx=1:numel(inports)
        port_name = get_param(inports(idx), 'Name');
        inports_names{idx} = port_name{1};
    end
else
    inports_names{1} = 'No available inputs.';
end
end
