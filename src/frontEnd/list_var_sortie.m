...
for k1=1:unbloc.num_output
	[is_bus bus] = BusUtils.is_bus(unbloc.outports_dt{k1});
	if is_bus
		...
		if dims(idx_dim) == -2
			...
			for idx=1:nb_bus_fields
				...
				[nb_dims dims_tmp] = Utils.get_port_dims(dims((idx_dim+2:numel(dims))), 1);
			...
			num_pred = num_pred + (dim_r * dim_c);
			[nb_dims dims_tmp] = Utils.get_port_dims(dims((idx_dim:numel(dims))), 1);
			idx_dim = idx_dim + nb_dims + 1;
		end
	end
end
