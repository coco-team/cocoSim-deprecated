...
if ~isFunction && strcmp(state.Decomposition,'PARALLEL_AND')
    ...
    [state_code, node_struct] = state_IDL(chart, state,state_name, state, variables_struct, node_struct, global_nodes_struct);
    states_code_array = [states_code_array, state_code];
else
...
%     end
    for i=1:number_children
        ...
        transitions_unless_code = [transitions_unless_code, condition_state ' restart ',upper(state_name), '\n\n']  ;
        [state_code, node_struct] = state_IDL(chart, state,state_name, source, variables_struct, node_struct, global_nodes_struct);
        states_code_array = [states_code_array, state_code];
    end    
end
